...
// the fly.
//
// `tsConfigPath` is what makes the `@/*` alias resolve: without it every
// aliased import comes back unresolved and the graph is mostly empty.
...
module.exports = createDepCruiserConfig({
  ...
  orphanExemptions: [
    // Vitest collects these by glob; no module imports a test file.
    '\\.test\\.(ts|tsx)$',
